...
public class Main {
    public static void main(String[] args) {
        ...
        students.stream()
                ...
                .sorted((lhs, rhs) -> Integer.compare(lhs.year(), rhs.year()))
                .distinct()
                .filter(book -> book.year() > 2000)
                .limit(3)
                // .map(book -> book.getYear())
                .findFirst()
                .ifPresentOrElse(book -> System.out.println("First found book is " + book.title() + " released in " + book.year() + "."), () -> System.out.println("No books released after 2000."));
    }
}
